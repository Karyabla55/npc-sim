...
class PerceptionSystem:
    ...
    def tick(self, stimuli: list[Stimulus], npc_position: SimVector3,
        ...
        for p in self._percepts.values():
            p.mark_not_visible()
        ...
        for stimulus in stimuli:
            ...
            # Upsert percept
            existing = self._percepts.get(stimulus.source_id)
            if existing is not None:
                ...
                self._percepts[stimulus.source_id] = existing

            changed.append(existing)

        # Prune expired
        self._percepts = {k: v for k, v in self._percepts.items()
                          if not v.is_expired(current_time, self.percept_timeout)}

        return changed

    def get_threats(self) -> list[PerceivedObject]:
        result = [p for p in self._percepts.values()
    ...
    def get_allies(self) -> list[PerceivedObject]:
        return [p for p in self._percepts.values() if p.tag.lower() == "ally"]
    ...
    def _find_percept(self, source_id: str) -> PerceivedObject | None:
        return self._percepts.get(source_id)
